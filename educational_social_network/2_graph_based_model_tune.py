...
''' Evaluation pipeline '''
def evaluate_with_set(rec_engine, train_set, content_type = 'c'):
    mrrs_10 = []
...
df_list = np.arange(0.1, 0.5, 0.1)
mrr_list = []
for df in df_list:
    recommender = RecommendationEngine(build_graph(parameter_dict_from_vector(np.ones(22))), df)
    mrr_list.append(evaluate_with_set(recommender, train_file, content_type))
