...
def parameter_dict_from_vector(vector):
    return {
        ...
        "W_USER_COURSE" : vector[1],
        "W_USER_POST" : vector[2],
        "W_USER_SOURCE" : vector[3],
        "W_USER_POST_COMMENT" : vector[4],
        "W_USER_POST_LIKE" : vector[5],
        "W_USER_POST_COMMENT_LIKE" : vector[6],
        "W_USER_SOURCE_RATING" : vector[7], #times the rating score
        "W_COMMENT_POST": vector[8],
        "W_USER_ORIGIN": vector[9],
        "W_USER_CITY": vector[10]
        }
...
class InteractionGraph:
    ...
    def add_edges_from_array(self, array, weight_front, additional_weight_collumn=False):
...
def build_graph(parameter_dictionary):
    ...
    multigraph.add_edges_from_array(user_follow_array, parameter_dictionary["W_USER_USER"])
    multigraph.add_edges_from_array(courses_follows_array, parameter_dictionary["W_USER_COURSE"])
    multigraph.add_edges_from_array(general_posts_array, parameter_dictionary["W_USER_POST"])
    multigraph.add_edges_from_array(general_sources_array, parameter_dictionary["W_USER_SOURCE"])
    multigraph.add_edges_from_array(ratings_array, parameter_dictionary["W_USER_SOURCE_RATING"], True)
    multigraph.add_edges_from_array(post_likes_array, parameter_dictionary["W_USER_POST_LIKE"])
    multigraph.add_edges_from_array(post_comments_array, parameter_dictionary["W_USER_POST_COMMENT"])
    multigraph.add_edges_from_array(comments_to_posts_array, parameter_dictionary["W_COMMENT_POST"])
    multigraph.add_edges_from_array(post_comment_likes_array, parameter_dictionary["W_USER_POST_COMMENT_LIKE"])
    multigraph.add_edges_from_array(user_origin_array, parameter_dictionary["W_USER_ORIGIN"])
    multigraph.add_edges_from_array(user_city_array, parameter_dictionary["W_USER_CITY"])
...
class RecommendationEngine:
    ...
    def __init__(self, multigraph, damping_factor = 0.3):
        ...
        for e in multigraph.graph.edges:
            ...
            if e[1] in self.user_follow_dict:
                self.user_follow_dict[e[1]].add(e[0])
...
def evaluate_with_set(rec_engine, test_set, content_type = 'c'):
    ...
    # generate N recommendations for this user
        ...
        recommendation_list_5 = recommendation_list[:5]
    ...
    # calculate MRR 
        ...
        if content in recommendation_list_10:
            ...
            hr_10 = 1
            if content in recommendation_list_5:
                mrr_5 = 1.0/(1+recommendation_list_5.index(content))
                hr_5 = 1
        mrrs_5.append(mrr_5)
...
recommender = RecommendationEngine(build_graph(parameter_dict_from_vector(np.ones(11))), df)
