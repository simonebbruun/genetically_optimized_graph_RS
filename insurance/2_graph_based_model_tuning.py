...
purchase_items_t = pd.read_csv('purchase_items_tr.csv')
...
def gen_evaluate_with_set(graph, train_set, k_neighbors, damping_factor, content_set, filter_dict):
    ...
    evaluation = Parallel(n_jobs=num_cores)(delayed(process)(graph, graph_nodes, train_set, user, k_neighbors, damping_factor, content_set, filter_dict) for user in np.unique(train_set[:,1]))
...
mrr_3 = gen_evaluate_with_set(rec_graph, train_file, k, df, content_file, train_filter)
